...
export class TradeFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (!this.receiveData?.currencies?.results) {
      console.warn("Currencies data is missing.");
    }
    this.currencies = this.receiveData?.currencies?.results ?? [];

    if (!this.receiveData?.products?.results) {
      console.warn("Products data is missing.");
    }
    this.products = this.receiveData?.products?.results ?? [];

    if (!this.receiveData?.customers?.results) {
      console.warn("Customers data is missing.");
    }
    this.customers = this.receiveData?.customers?.results ?? [];
